...
class RenewalInteraktEvent(models.Model):
    ...
    _rec_name = "event_id"  # [MIGRATION] Added for UI clarity
    ...
    template_name = fields.Char(string="Template Name", compute="_parse_raw_payload", store=True, index=True)

    # [FIX] New Odoo 19 Constraint Syntax
    _event_id_owner_id_uniq = models.Constraint(
        'UNIQUE(event_id, owner_id)',
        message='The Event ID must be unique per owner.'
    )

    @api.depends('raw_payload')
    def _parse_raw_payload(self):
        """ Parses the raw JSON payload to extract template name"""
        for event in self:
            ...
            try:
                ...
                _logger.error(f"Failed to parse raw_payload for RenewalInteraktEvent ID {event.id}: {e}")