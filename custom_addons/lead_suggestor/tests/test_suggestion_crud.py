...
class TestSuggestionCRUD(PropertyInventoryTestCase):
    ...
    def test_01_create_suggestion_with_required_fields(self):
        ...
        self.assertEqual(suggestion.property_base_id, prop)
        self.assertEqual(suggestion.status, "new")

    def test_02_suggestion_without_property_is_allowed(self):
        """property_base_id is optional (used during migration before backfill).
        Suggestions can exist without a linked property.base record.
        """
        suggestion = self.env["property.lead.suggestion"].create(
            {
                "suggested_lead_phone": "9876543210",
                "lead_name": "Orphan Lead",
                "property_tag": "NO-PROP-TAG",
                "status": "new",
            }
        )
        self.assertTrue(suggestion.id)
        self.assertFalse(suggestion.property_base_id)
    ...
    def test_06_property_tag_stored_on_suggestion(self):
        """Property tag should be stored directly on the suggestion record."""
