...
class TestSuggestionWhatsapp(PropertyInventoryTestCase):
    ...
    def test_01_whatsapp_url_10_digit_phone(self):
        ...
        self.assertIn(
            "phone=919876543210", suggestion.suggested_lead_phone_whatsapp_url
        )
    ...
    def test_11_action_whatsapp_message_contains_details(self):
        ...
        self.assertIn("Downtown", message)  # Location cleaned (A- prefix stripped)
        self.assertIn("Mumbai", message)
        # property_link is computed from name+prop_id — verify it appears if non-empty
        if prop.property_link:
            self.assertIn(prop.property_link, message)
