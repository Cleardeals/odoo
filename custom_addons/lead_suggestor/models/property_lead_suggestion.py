...
SUGGESTIONS_TABLE_ID = (
    "cleardeals-459513.active_to_active.suggested_leads_for_properties"
)
...
class PropertyLeadSuggestion(models.Model):
    ...
    def _compute_generation_date_display(self):
        ...
        for rec in self:
            rec.generation_date_display = (
                rec.generation_date.strftime("%d/%m/%Y") if rec.generation_date else ""
            )
    ...
    def _compute_suggested_lead_phone_whatsapp_url(self):
        ...
        for rec in self:
            ...
            if number_to_use:
                ...
                rec.suggested_lead_phone_whatsapp_url = (
                    f"whatsapp://send?phone={number_to_use}"
                )
            else:
    ...
    def action_log_feedback(self):
        ...
        return {
            ...
            "name": _("Log Feedback for %s")
            % (self.lead_name or self.suggested_lead_phone),
                ...
                vals_to_create.append(
                    {
                        ...
                        "original_property_similarity": (
                            row.original_property_similarity or 0.0
                        )
                        * 100.0,
