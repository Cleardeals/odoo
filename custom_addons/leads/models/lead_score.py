...
class LeadScore(models.Model):
    ...
    state = fields.Selection(
        [
            ...
            ("no_requirements", "No Requirements"),
            (
                "details_shared_and_interested_for_site_visit",
                "Details Shared and Interested for Site Visit",
            ),
            ("switched_off", "Switched Off"),
            ("requirement_closed", "Requirement Closed"),
            ("property_sold_out", "Property Sold Out"),
    ...
    def _compute_is_actionable_today(self):
        ...
        today = date.today()
        for lead in self:
            lead.is_actionable_today = (not lead.next_follow_up_date) or (
    ...
    def _compute_next_follow_up_date(self):
        ...
        for lead in self:
            if (
                ...
                lead.next_follow_up_date = lead.site_visit_scheduled_date + timedelta(
                    days=1,
                )
            else:
    ...
    def _onchange_state_set_follow_up(self):
        ...
        if (
            ...
            self.next_follow_up_date = fields.Date.context_today(self) + timedelta(
                days=1,
            )
    ...
    def action_open_bigquery_wizard(self):
        ...
        action = self.env["ir.actions.act_window"]._for_xml_id(
            "leads.action_fetch_leads_wizard",
        )
        return action
