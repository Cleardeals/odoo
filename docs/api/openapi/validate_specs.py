...
# Paths that are documented on purpose even though no route in this checkout
# serves them: they belong to modules that live on another branch. Keeping them
# here means the docs stay complete without the coverage check going red.
OFF_BRANCH_PATHS: dict[str, str] = {
    "/wa/media/upload": "wa_communication, on feature/pub-sub",
    "/wa/pubsub/push": "cleardeals_pubsub, on feature/pub-sub",
}
...
def check_coverage(specs: dict[Path, dict], code_routes: dict[str, str]) -> list[str]:
    ...
    for path, spec_files in sorted(documented.items()):
        if path not in code_routes and path not in OFF_BRANCH_PATHS:
            problems.append(
