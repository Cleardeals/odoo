...
Published to the **Nirat Patel's Workspace** Postman workspace (workspace id
`8acc947d-2254-4a93-8d8a-5de4ad275ed1`). Each spec lives in Spec Hub and its
collection was generated from that spec, so the collections are regenerated, not
hand-edited, whenever a spec changes.

| Spec (Spec Hub) | Collection | Environment |
|---|---|---|
| `09cdca4f-c23e-4dd4-8bb1-9bc9591b6c34` Lead Intake | `54fd96e4-406e-41bb-a6b6-8ba5fe50b3df` | Cleardeals — Lead Intake (Staging) |
| `995dc399-b79a-495d-9367-29c67ca70008` Properties | `66218fbf-80a7-4fc4-96e9-1bff16653e37` | Cleardeals — Properties API (Staging) |
| `5e4148a5-2ed0-4f7c-bf32-c6247bab5660` Track | `2894d8a1-2421-4fe8-b948-f140d9d4d959` | Cleardeals — Track API (Staging) |
| `ec322697-0703-4b43-bc9c-acd1d21ba6f1` Internal | `0f5cd5d2-f7d0-44ea-abd0-1a604748308e` | Cleardeals — Internal API (Staging) |

Requests are foldered by spec tag and carry the example bodies and query values
from the spec, so most are runnable as soon as an environment is selected.

**One environment per collection, deliberately.** The generator names every
API-key variable `{{apiKey}}`, and the four APIs hold four different secrets, so
a single shared environment could not serve them. The Lead Intake requests were
repointed to per-portal variables (`{{magicbricksApiKey}}`, `{{acres99ApiKey}}`,
`{{squareyardsApiKey}}`, `{{cleardealsLeadApiKey}}`) because that one collection
carries four secrets of its own.

Every key variable ships **empty and marked secret**. Fill them from the Odoo
system parameters named in each spec's security block; do not commit values.

Set `host` and `baseUrl` to the deployment you are hitting — they default to the
placeholder `staging.cleardeals.example`, which resolves to nothing.

To regenerate after editing a spec, push the spec file to Spec Hub and generate
the collection from it again, rather than editing requests in the Postman UI.
